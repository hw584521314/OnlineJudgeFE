<template>
<div class="view">
    <Panel :title="this.$i18n.t('m.Exam_Problem_List')">
      <div slot="header">
        <el-input
          v-model="keyword"
      ...
      <div class="panel-options">
        ...
        <el-button v-if="examId" type="primary"
                   size="small" icon="el-icon-plus"
                   @click="addProblemDialogVisible = true">从公共题库中添加题目
        </el-button>
        <el-pagination
          class="page"
    ...
    <el-dialog title="添加考试题目"
               v-if="examId"
               width="80%"
               :visible.sync="addProblemDialogVisible"
               @close-on-click-modal="false">
      <add-problem-component :examID="examId" @on-change="getProblemList"></add-problem-component>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      goEdit (problemId) {
          ...
          this.$router.push({name: 'edit-exam-problem', params: {problemId: problemId, examId: this.examId}})
        
      },
      goCreateProblem () {

          this.$router.push({name: 'create-exam-problem', params: {examId: this.examId}})
      ...
      getProblemList (page = 1) {
        ...
        api["getExamProblemList"](params).then(res => {
          this.loading = false
      ...
      updateProblem (row) {
        ...
        api['editExamProblem'](data).then(res => {
